 ...
 */
fun main() {
    Day03("day03input.txt").run()
}

class Day03(fileName: String) : BaseDay(fileName) {
    override fun runPart1(): Int {
        return findNumbers(lines).sum()
    }

    override fun runPart2(): Int {
        return findGears(lines).sum()

    }

    internal fun findNumbers(lines: List<String>): List<Int> {
    ...
    internal fun findGears(lines: List<String>): List<Int> {
        val result = mutableListOf<Int>()
     ...
     */
    internal fun findSymbols(line: String): List<Int> {
        val result = mutableListOf<Int>()
     ...
     */
    private fun findGearsSymbols(line: String): List<Int> {
        val result = mutableListOf<Int>()
        line.forEachIndexed { index, c ->
            if (c == '*') {
                result.add(index)
     ...
     */
    internal fun extractNumber(lines: List<String>, lineIndex: Int, index: Int, touchedNumbers: Array<BooleanArray>): Int {
        var result = ""
        var i = index
