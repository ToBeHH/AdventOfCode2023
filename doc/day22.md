...
[![](https://i.redd.it/0ji12if3bv7c1.png)](https://www.reddit.com/r/adventofcode/comments/18ohyg8/2023_day_22ai_art/)
...
A line like `0,0,1~0,0,10` represents a **ten-cube brick** which is oriented **vertically**. One end of the brick is the
...
need to start by figuring out where they will end up. Bricks are magically stabilized, so they **never rotate**, even in
weird situations like where a long horizontal brick is only supported on one end. Two bricks cannot occupy the same
...
- Brick `F` cannot be disintegrated; the brick above it (`G`)
  would fall.
